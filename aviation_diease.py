...
# categorical_features = ['Age_category_IMMATURE','Age_category_UNKNOWN/OTHER']
X_train_enc = pd.get_dummies(X_train, columns=categorical_features, drop_first=True,dtype=int)
X_test_enc = pd.get_dummies(X_test, columns=categorical_features, drop_first=True,dtype=int)
...
stepwise_results = []
features = ['age_cat_Monthly_Percent','age_cat_Yearly_Percent','rolling_3m', 'rolling_6m']
...
features = ['Region', 'month_sin', 'month_cos', 'Temp', 'sea_level', 'Age_category',
...
categorical = ['Region', 'Age_category']
